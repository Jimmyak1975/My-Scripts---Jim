...
import pandas as pd
import numpy as np
import psutil  # Modification: Added psutil for memory monitoring
...
def setup_logging():
    ...
    # File Handler
    fh = logging.FileHandler('futures_1_2_alerts.log', mode='a')  # Modification: Changed log filename for clarity
...
class GlobalParams:
    ...
    """
    TOP_GAINERS_LIMIT = 30              # Modification: Increased to 30 top gainers
    TOP_LOSERS_LIMIT = 30               # Modification: Added 30 top losers
    ...
    TIMEFRAMES = [1, 2]  # Modification: Updated to include only 1m and 2m timeframes
    BINANCE_INTERVALS = {
        1: '1m'  # Modification: Removed '3m' timeframe
    }
...
def immediate_price_change_alert(df: pd.DataFrame, threshold=0.5) -> tuple:
    ...
    if abs(pct_change) >= threshold:  # Modification: Ensure absolute percentage change
...
# Modification: Implemented Telegram message queue and rate limiter
telegram_queue = asyncio.Queue()
telegram_semaphore = asyncio.Semaphore(30)  # Modification: Added semaphore for rate limiting
...
# =========================
async def fetch_futures_top_gainers_and_losers(client: AsyncClient, gainers_limit=30, losers_limit=30, retries=3, backoff=1) -> list:
    """
    Fetch the top gainers and top losers from Binance Futures based on 24-hour price change percentage with retry.
    ...
    Returns:
        list: Combined list of top gainers and top losers symbols.
    """
    for attempt in range(1, retries + 1):
        try:
            logging.info("Fetching all FUTURES tickers (24h).")
            tickers = await client.futures_ticker()
            df_tickers = pd.DataFrame(tickers)
            df_tickers = df_tickers[df_tickers['symbol'].str.endswith('USDT')]  # Modification: Ensure USDT-paired
            ...
            combined_symbols = list(set(top_gainers + top_losers))  # Remove duplicates if any
            logging.info(f"Top Gainers (FUTURES): {top_gainers}")
            logging.info(f"Top Losers (FUTURES): {top_losers}")
            return combined_symbols
        except Exception as e:
            logging.error(f"Attempt {attempt} - Error fetching FUTURES top gainers and losers: {e}", exc_info=True)
            if attempt < retries:
                wait_time = backoff * (2 ** (attempt - 1))
                logging.info(f"Retrying in {wait_time} seconds...")
                await asyncio.sleep(wait_time)
            else:
                logging.error(f"All {retries} attempts failed to fetch top gainers and losers.")
                sentry_sdk.capture_exception(e, fingerprint=["fetch_futures_top_gainers_and_losers"])
                return []
    ...
    try:
        ...
        for symbol in symbols:
            ...
            for timeframe in GlobalParams.TIMEFRAMES:
                ...
                fetch_tasks.append(fetch_futures_klines(symbol, interval, client, limit=50))
        ...
        for symbol, timeframes in symbol_data.items():
            for timeframe, df in timeframes.items():
                ...
                if direction and abs(pct_change) >= GlobalParams.PRICE_CHANGE_THRESHOLD:  # Modification: Ensure price change threshold
        ...
        # Modification: Send aggregated alerts
...
async def main_loop(client: AsyncClient, bot: Bot, chat_id: str, db: AlertedTokensDB):
    ...
    # Initialize last_gainers_update
    last_gainers_update = datetime.now(timezone.utc) - timedelta(seconds=GlobalParams.TOP_GAINERS_UPDATE_INTERVAL)

    # Modification: Start Telegram worker
    asyncio.create_task(telegram_worker())

    # Modification: Start memory monitoring
    ...
    while True:
        try:
            ...
            # Update top gainers and losers every TOP_GAINERS_UPDATE_INTERVAL seconds
            if not TOKENS or (now - last_gainers_update).total_seconds() >= GlobalParams.TOP_GAINERS_UPDATE_INTERVAL:
                new_tokens = await fetch_futures_top_gainers_and_losers(
                    client,
...
# Modification: Added memory monitoring function
...
async def main():
    ...
    """
    # Modification: Implement retry mechanism for Binance client creation
    retries = 5
    backoff = 5
    client = None
    for attempt in range(1, retries + 1):
