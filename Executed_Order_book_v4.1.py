...
# NOTE: This can theme the Matplotlib toolbar area (bottom "settings" row).
# The OS title bar color (topmost Windows bar) cannot be controlled by Matplotlib.
def _apply_mpl_toolbar_theme(fig, bg="#1E1E1E", fg="#D0D0D0"):
    try:
        ...
        # TkAgg toolbar (NavigationToolbar2Tk)
        if "tk" in backend:
            try:
                # Toolbar itself
                ...
                # Children widgets (buttons/labels)
        ...
        # QtAgg toolbar (QToolBar)
        elif "qt" in backend:
            try:
                # QToolBar: stylesheet controls background + text
                tb.setStyleSheet(f"""
                    QToolBar {{
                        background: {bg};
                        color: {fg};
        ...
        # Other backends: nothing to do safely
        else:
            return
...
# ✅ Dark grey background for the canvas area
fig.patch.set_facecolor(BASE_COLOR_ODD)
ax.set_facecolor(BASE_COLOR_ODD)

# ✅ Theme the bottom Matplotlib toolbar (dark bg + light text/icons)
...
def update(frame):
    ...
    # ✅ Re-apply dark background after clear()
    fig.patch.set_facecolor(BASE_COLOR_ODD)
    ax.set_facecolor(BASE_COLOR_ODD)
