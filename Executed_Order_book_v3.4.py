...
def update(frame):
    ...
    # Create the totals row (formatted with commas)
    totals_row = [f"{int(round(total_sell)):,}" if total_sell > 0 else "",
                  f"{int(round(total_buy)):,}" if total_buy > 0 else ""]
    
    # Create rows for individual trades
    trade_rows = [
    ...
    ax.clear()
    ax.axis('off')
    table = ax.table(cellText=table_data, colLabels=["SELL", "BUY"], loc='center', cellLoc='center')
    ...
    for key, cell in table.get_celld().items():
        ...
        if row == 0:  # Header row
            ...
            cell.set_text_props(color="red" if col == 0 else "green", weight='bold')
    
    # Apply new highlight thresholds on individual trade cells (skip header and totals row)
    for i in range(2, len(table_data) + 1):
        trade_index = i - 2  # Adjust index for our display lists
        for col, val in [(0, sell_values[trade_index]), (1, buy_values[trade_index])]:
            cell = table[i, col]
            if val > 500000:
