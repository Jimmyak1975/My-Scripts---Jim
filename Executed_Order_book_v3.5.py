...
# ---------------- Configuration ----------------
# Using SOL vs USDT pair for all exchanges
SYMBOL = "SOLUSDT"         
...
def fetch_exchange_trades(exchange, url):
    ...
    try:
        if exchange == "Binance":
            ...
            # Kraken returns data under a key matching the pair (e.g., "SOLUSD")
            ...
            params = {"limit": TOTAL_FETCH_LIMIT}
            response = requests.get(url, params=params, timeout=10).json()
            # Bitfinex returns each trade as [ID, MTS, AMOUNT, PRICE]
            for trade in response:
                if len(trade) >= 4:
                    price = float(trade[3])
