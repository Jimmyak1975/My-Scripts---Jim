...
def setup_logging():
    ...
    # File Handler
    fh = logging.FileHandler('futures_gainers_losers_v12.log', mode='a')
...
class AlertedTokensDB:
    ...
    def migrate_tables(self):
        ...
        """
        try:
            # Check if MA99 exists in indicators table
            self.cursor.execute("PRAGMA table_info(indicators);")
            columns = [info[1] for info in self.cursor.fetchall()]
            if 'MA99' not in columns:
                self.cursor.execute('ALTER TABLE indicators ADD COLUMN MA99 REAL;')
                self.conn.commit()
                logging.info("Migrated 'indicators' table to add 'MA99' column.")
        except Exception as e:
        ...
        try:
            self.cursor.execute('''
                INSERT INTO indicators (
                    symbol, timeframe, timestamp, MA7, MA25, MA99, EMA12, EMA26, MACD, Signal_Line, confidence
                ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            ''', (
                symbol,
                timeframe,
...
def calculate_confidence(df: pd.DataFrame) -> float:
    ...
    if all(col in df.columns for col in ['MA7', 'MA25', 'MA99']):
        ...
        ma25 = df['MA25'].iloc[-1]
        ma99 = df['MA99'].iloc[-1]
        if pd.notna(ma7) and pd.notna(ma25) and pd.notna(ma99):
            if ma7 > ma25 > ma99:
                bullish_score += 3
            elif ma7 < ma25 < ma99:
                bearish_score += 3
...
async def fetch_futures_klines(symbol: str, interval_str: str, client: AsyncClient, limit=200, retries=3, backoff=1) -> pd.DataFrame:
    """
    ...
    Parameters:
        ...
        client (AsyncClient): Instance of Binance AsyncClient.
        limit (int, optional): Number of klines to fetch. Defaults to 200.
        retries (int): Number of retry attempts.
        backoff (int): Initial backoff time in seconds.
    ...
    try:
        ...
        for symbol in symbols:
            ...
            for timeframe in GlobalParams.TIMEFRAMES:
                ...
                task = fetch_futures_klines(symbol, interval, client, limit=200 if timeframe == 1 else 200)
