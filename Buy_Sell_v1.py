...
# ======================================================
# User-Defined Week Period (Monday to Sunday)
# ======================================================
# Specify the start date (must be a Monday) in "YYYY-MM-DD" format.
WEEK_START = "2025-02-10"  # Example: Monday, Feb 9, 2025
# Compute the end date as 6 days later (i.e., Sunday)
start_dt = pd.to_datetime(WEEK_START)
WEEK_END = (start_dt + pd.Timedelta(days=6)).strftime("%Y-%m-%d")
print(f"Analyzing week from {WEEK_START} to {WEEK_END}")

# Convert WEEK_START and WEEK_END to timestamps (in ms) assuming local time (UTC+2)
# For WEEK_START, use midnight of that day.
local_tz = timezone(timedelta(hours=2))
week_start_local = pd.to_datetime(WEEK_START).tz_localize(local_tz)
start_time_ms = int(week_start_local.timestamp() * 1000)
# For WEEK_END, cover the entire day (set time to 23:59:59)
week_end_local = pd.to_datetime(WEEK_END, format="%Y-%m-%d").tz_localize(local_tz) + pd.Timedelta(hours=23, minutes=59, seconds=59)
...
def process_data_for_hourly_diff(raw_data):
    ...
    4. Filters out rows with open_time_local before WEEK_START.
    ...
    # Filter out rows with open_time_local before WEEK_START (we needed extra candle only for calculation)
...
def analyze_patterns_hourly_diff(df_hourly):
    """
    1. Groups data by (day_of_week, hour) to compute the average hourly_pct_change over the week.
...
if __name__ == "__main__":
    # Collect data for the specified week period (with an extra hour before start for calculation)
