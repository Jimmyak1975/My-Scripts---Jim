...
def setup_logging():
    ...
    # File Handler
    fh = logging.FileHandler('futures_gainers_losers_v13.log', mode='a')
...
class GlobalParams:
    ...
    TOP_SYMBOLS_UPDATE_INTERVAL = 180   # 3 minutes in seconds
    POLL_INTERVAL = 10                  # Reduced polling interval to 10 seconds
    PRICE_CHANGE_THRESHOLD = 0.2        # Reduced to 0.2% immediate price change
    
    # Confidence thresholds
    CONFIDENCE_HIGH = 50.0              # Lowered from 55 to 50
    CONFIDENCE_LOW = 40.0               # Raised from 45 to 40
    ...
    BINANCE_INTERVALS = {
        1: '1m',
        3: '3m'
        # Removed '2m' as it's not supported by Binance API
    }

# =========================
# 6. SQLite Database Setup (Enhanced with Migration)
# =========================
class AlertedTokensDB:
    ...
    def migrate_tables(self):
        """
        Migrate tables to include new columns if they don't exist.
        """
        try:
            # Check if MA50 exists in indicators table
            self.cursor.execute("PRAGMA table_info(indicators);")
            columns = [info[1] for info in self.cursor.fetchall()]
            if 'MA50' not in columns:
                self.cursor.execute('ALTER TABLE indicators ADD COLUMN MA50 REAL;')
                self.conn.commit()
                logging.info("Migrated 'indicators' table to add 'MA50' column.")
        except Exception as e:
            logging.error(f"Error migrating SQLite tables: {e}", exc_info=True)
            sentry_sdk.capture_exception(e)  # Capture exception in Sentry
        ...
        try:
            self.cursor.execute('''
                INSERT INTO indicators (
                    symbol, timeframe, timestamp, MA7, MA25, MA50, EMA12, EMA26, MACD, Signal_Line, confidence
                ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            ''', (
                symbol,
                timeframe,
...
def calculate_ma_and_macd(df: pd.DataFrame, timeframe: int) -> pd.DataFrame:
    """
    Calculate Moving Averages (MA) and MACD indicators based on timeframe.
    ...
    if timeframe == 1:
        ...
        df['MA50'] = np.nan  # MA50 not applicable for higher timeframes
...
def calculate_confidence(df: pd.DataFrame, timeframe: int) -> float:
    """
    ...
    # Moving Averages
    if timeframe == 1 and all(col in df.columns for col in ['MA7', 'MA25', 'MA50']):
        ma7 = df['MA7'].iloc[-1]
        ma25 = df['MA25'].iloc[-1]
        ma50 = df['MA50'].iloc[-1]
        if pd.notna(ma7) and pd.notna(ma25) and pd.notna(ma50):
            if ma7 > ma25 > ma50:
                bullish_score += 3
            elif ma7 < ma25 < ma50:
                bearish_score += 3
    elif timeframe in [2, 3] and all(col in df.columns for col in ['MA7', 'MA25']):
        ma7 = df['MA7'].iloc[-1]
        ma25 = df['MA25'].iloc[-1]
        if pd.notna(ma7) and pd.notna(ma25):
            if ma7 > ma25:
                bullish_score += 3
            elif ma7 < ma25:
                bearish_score += 3
...
def immediate_price_change_alert(df: pd.DataFrame, threshold=0.2) -> tuple:
    """
...
# =========================
async def fetch_futures_exchange_info(client: AsyncClient) -> set:
    """
    Fetch exchange information to get a set of available symbols on Binance Futures.

    Parameters:
        client (AsyncClient): Instance of Binance AsyncClient.

    Returns:
        set: Set of available symbols.
    """
    try:
        ...
        logging.info(f"Fetched {len(symbols)} symbols from Binance Futures.")
        return symbols
    except Exception as e:
        logging.error(f"Error fetching exchange info: {e}", exc_info=True)
        sentry_sdk.capture_exception(e)
        return set()

async def fetch_futures_top_symbols(client: AsyncClient, valid_symbols: set, gainers_limit=30, losers_limit=30, retries=3, backoff=1) -> list:
    """
    Fetch the top gainers and losers from Binance Futures based on 24-hour price change percentage with retry.

    Parameters:
        client (AsyncClient): Instance of Binance AsyncClient.
        valid_symbols (set): Set of symbols available on Binance Futures.
    ...
    for attempt in range(1, retries + 1):
        try:
            ...
            df_tickers = df_tickers[df_tickers['symbol'].isin(valid_symbols)]  # Ensure symbols are valid
...
async def fetch_futures_klines(symbol: str, timeframe: int, client: AsyncClient, limit=75, retries=3, backoff=1) -> pd.DataFrame:
    """
    Fetch klines (candlestick data) from Binance Futures for a given symbol and timeframe with retry.

    Parameters:
        symbol (str): Trading symbol (e.g., BTCUSDT).
        timeframe (int): Timeframe in minutes.
        client (AsyncClient): Instance of Binance AsyncClient.
        limit (int, optional): Number of klines to fetch. Defaults to 75.
        retries (int): Number of retry attempts.
        backoff (int): Initial backoff time in seconds.
    ...
    """
    interval_str = GlobalParams.BINANCE_INTERVALS.get(timeframe)
    if not interval_str:
        if timeframe == 2:
            # For 2m timeframe, aggregate from 1m klines
            interval_str = '1m'
        else:
            logging.error(f"Unsupported timeframe: {timeframe}m")
            return pd.DataFrame()
    ...
    for attempt in range(1, retries + 1):
        try:
            ...
            if timeframe == 2:
                df = aggregate_2m_from_1m(df)
                if df.empty:
                    logging.warning(f"No data after aggregating 2m klines for {symbol}.")
            return df
        except Exception as e:
            ...
            if attempt < retries:
                ...
                sentry_sdk.capture_exception(e, fingerprint=["fetch_futures_klines", symbol, timeframe])
                return pd.DataFrame()
    ...
    try:
        ...
        for symbol in symbols:
            ...
            for timeframe in GlobalParams.TIMEFRAMES:
                task = fetch_futures_klines(symbol, timeframe, client, limit=75)
        ...
        for symbol, timeframes in symbol_data.items():
            for timeframe, df in timeframes.items():
                ...
                df = calculate_ma_and_macd(df, timeframe)
                ...
                indicators = {
                    'MA7': df['MA7'].iloc[-1],
                    'MA25': df['MA25'].iloc[-1],
                    'MA50': df['MA50'].iloc[-1] if timeframe == 1 else None,
                ...
                confidence = calculate_confidence(df, timeframe)
        ...
        for symbol, timeframes in symbol_data.items():
            for timeframe, df in timeframes.items():
                ...
                direction = None
                if confidence > GlobalParams.CONFIDENCE_HIGH and 'Hammer detected!' in alerts:
                    direction = 'buy'
                elif confidence < GlobalParams.CONFIDENCE_LOW and 'Bearish Engulfing detected!' in alerts:
                    direction = 'sell'
...
# =========================
async def main_loop(client: AsyncClient, bot: Bot, chat_id: str, db: AlertedTokensDB, valid_symbols: set):
    """
    ...
    last_symbols_update = datetime.now(timezone.utc) - timedelta(seconds=GlobalParams.TOP_SYMBOLS_UPDATE_INTERVAL)
    ...
    while True:
        try:
            ...
            # Update top symbols every TOP_SYMBOLS_UPDATE_INTERVAL seconds
            if not TOKENS or (now - last_symbols_update).total_seconds() >= GlobalParams.TOP_SYMBOLS_UPDATE_INTERVAL:
...
async def main():
    ...
    # Fetch exchange info to get valid symbols
    valid_symbols = await fetch_futures_exchange_info(client)
    if not valid_symbols:
        logging.critical("No valid symbols fetched from Binance Futures. Exiting.")
    ...
    try:
        await main_loop(client, bot, TelegramConfig.TELEGRAM_CHAT_ID, db, valid_symbols)
    except Exception as e:
