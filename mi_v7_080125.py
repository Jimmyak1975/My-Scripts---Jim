...
def setup_logging():
    ...
    # File Handler
    fh = logging.FileHandler('futures_1_2_3m_fast.log', mode='a')
...
class AlertedTokensDB:
    ...
    def initialize_table(self):
        """
        Create the alerted_tokens table if it doesn't exist.
        """
        try:
            self.cursor.execute('''
...
async def send_telegram_message(text: str, retries=3, backoff=1):
    """
    Send a message via Telegram bot with retry mechanism.
...
# =========================
async def fetch_futures_top_gainers(client: AsyncClient, limit=20, retries=3, backoff=1) -> list:
    """
    Fetch the top gainers from Binance Futures based on 24-hour price change percentage with retry.
    ...
    Returns:
        list: List of top gainers symbols.
    """
    for attempt in range(1, retries + 1):
        try:
            ...
            logging.error(f"Attempt {attempt} - Error fetching FUTURES top gainers: {e}", exc_info=True)
            if attempt < retries:
                wait_time = backoff * (2 ** (attempt - 1))
                logging.info(f"Retrying in {wait_time} seconds...")
                await asyncio.sleep(wait_time)
            else:
                logging.error(f"All {retries} attempts failed to fetch top gainers.")
                sentry_sdk.capture_exception(e, fingerprint=["fetch_futures_top_gainers"])
                return []
...
async def fetch_futures_klines(symbol: str, interval_str: str, client: AsyncClient, limit=50, retries=3, backoff=1) -> pd.DataFrame:
    ...
    Parameters:
        symbol (str): Trading symbol (e.g., BTCUSDT).
        interval_str (str): Kline interval (e.g., '1m', '2m', '3m').
        client (AsyncClient): Instance of Binance AsyncClient.
...
# =========================
# 11. Analyze Symbol Function
# =========================
async def analyze_symbol(symbol: str, client: AsyncClient, bot: Bot, chat_id: str, alerted_tokens: set, db: AlertedTokensDB):
    """
    ...
    Parameters:
        ...
        db (AlertedTokensDB): Instance of AlertedTokensDB for database operations.
    """
    try:
        # Check if the symbol has already been alerted
        ...
        # Fetch 1m and 3m klines with retries
        ...
        sentry_sdk.capture_exception(e, fingerprint=[f"analyze_symbol-{symbol}"])

# =========================
# 12. Main Polling Loop
# =========================
    ...
    while True:
        try:
            ...
            if not TOKENS or (now - last_gainers_update).total_seconds() >= GlobalParams.TOP_GAINERS_UPDATE_INTERVAL:
                new_tokens = await fetch_futures_top_gainers(client, limit=GlobalParams.TOP_GAINERS_LIMIT)
                if new_tokens:
                    TOKENS = new_tokens
            ...
            for symbol in TOKENS:
                if symbol in alerted_tokens:
                    continue
                tasks.append(analyze_symbol(symbol, client, bot, chat_id, alerted_tokens, db))
...
async def main():
    ...
    """
    # Initialize Binance client without API keys for public data access
    try:
        client = await AsyncClient.create()
        logging.info("Binance AsyncClient initialized without API keys.")
    except Exception as e:
        logging.error(f"Error initializing Binance client: {e}", exc_info=True)
        sentry_sdk.capture_exception(e, fingerprint=["AsyncClient.create"])
        return  # Exit the main function if client initialization fails
