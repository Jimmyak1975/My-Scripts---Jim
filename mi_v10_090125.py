...
sentry_sdk.init(
    dsn="https://d6324dff11af3c2f1e2aa160fd9efa6c@o4508608695566336.ingest.us.sentry.io/4508608760643584",
...
class GlobalParams:
    ...
    TIMEFRAMES = [1, 2, 3]
    # Exclude '2m' since Binance does not support it directly
    BINANCE_INTERVALS = {
        1: '1m',
        3: '3m'
    }
        ...
        for symbol, timeframes in symbol_data.items():
            for timeframe, df in timeframes.items():
                ...
                if direction:
                    ...
                    logging.info(f"Alert prepared for {symbol}: {direction.upper()}")
...
async def main_loop(client: AsyncClient, bot: Bot, chat_id: str, db: AlertedTokensDB):
    """
    The main polling loop that periodically fetches top gainers and analyzes all symbols in batches.
