...
COINS = [
    ...
    "SUIUSDT", "ENAUSDT", "ONDOUSDT", "HBARUSDT", "AVAXUSDT", "AAVEUSDT", "1000PEPEUSDT"
]
    ...
    try:
        ...
        if "data" in data and data["data"].get("e") == "kline":
            ...
            if last_direction != 0 and new_direction != last_direction:
                ...
                # Use a different threshold for BTCUSDT (0.1%) vs. other coins (0.5%)
                threshold = 0.1 if symbol == "BTCUSDT" else 0.5
                if abs(cumulative_percent) >= threshold:
                    if now_ts - cumulative_data[symbol]["last_notif_timestamp"] < 180:
                        return
                    
                    # Update last_notif_timestamp *before* sending the message to enforce cooldown immediately
