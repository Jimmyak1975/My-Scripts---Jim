...
# ======================================================
# User-Defined Month Period (Monday to Sunday)
# ======================================================
# Specify the start date for the 1-month period in "YYYY-MM-DD" format.
# (Ideally, choose a Monday to view a full week structure)
MONTH_START = "2025-01-06"  # Example: January 1, 2023

# Compute the end date as one month later minus one day (covers the entire month)
start_dt = pd.to_datetime(MONTH_START)
MONTH_END = (start_dt + pd.DateOffset(months=1) - pd.Timedelta(days=1)).strftime("%Y-%m-%d")
print(f"Analyzing month from {MONTH_START} to {MONTH_END}")

# Convert MONTH_START and MONTH_END to timestamps (in ms) using local time (UTC+2)
local_tz = timezone(timedelta(hours=2))
month_start_local = pd.to_datetime(MONTH_START).tz_localize(local_tz)
start_time_ms = int(month_start_local.timestamp() * 1000)
# For MONTH_END, cover the entire day (set time to 23:59:59)
month_end_local = pd.to_datetime(MONTH_END, format="%Y-%m-%d").tz_localize(local_tz) + pd.Timedelta(hours=23, minutes=59, seconds=59)
end_time_ms = int(month_end_local.timestamp() * 1000)
...
def process_data_for_hourly_diff(raw_data):
    ...
    4. Filters out rows with open_time_local before month_start_local.
    ...
    # Filter out rows with open_time_local before month_start_local
    df = df[df["open_time_local"] >= month_start_local]
    
    return df[["date", "day_of_week", "hour", "hourly_pct_change"]]

def analyze_patterns_hourly_diff(df_hourly):
    """
    1. Groups data by (day_of_week, hour) to compute the average hourly_pct_change over the 1-year period.
    ...
    plt.title(
        f"Hourly % Change from Previous Hour (Avg) - {SYMBOL} (Local Time UTC+2)\n"
        f"Month: {MONTH_START} to {MONTH_END}"
    )
...
if __name__ == "__main__":
    # Collect 1-month data (365 days replaced with 1 month period)
